...
async function tg(token, method, body){
  ...
  const text = await res.text().catch(()=> ''); // лог короткий
...
function okSecret(req, exp){ if(!exp) return true; const got=req.headers.get('X-Telegram-Bot-Api-Secret-Token'); return !!got && got===exp }
...
async function handleUpdate(u, env){
  try{
    ...
    const reply = text?.toLowerCase()==='ping' ? 'pong' : (text?`Ти написав: ${text}`:'Привіт! Я на звʼязку ✅');
    if(!env.BOT_TOKEN){ console.error('ENV BOT_TOKEN missing'); return }
    await tg(env.BOT_TOKEN,'sendMessage',{chat_id:chat,text:reply});
  }catch(e){ console.error('handleUpdate error', String(e)) }
}
...
export default {
  async fetch(req, env, ctx){
    ...
    if(req.method==='GET' && url.pathname==='/health'){
      return json({ok:true, hasBOT_TOKEN: !!env.BOT_TOKEN, hasWEBHOOK_SECRET: !!env.WEBHOOK_SECRET});
    }
    if(req.method==='GET' && url.pathname==='/diag'){
      return new Response('ok',{status:200}); // просто пінг
    }
    if(req.method==='POST' && url.pathname==='/webhook'){
      if(!okSecret(req, env.WEBHOOK_SECRET)){ console.warn('Bad secret'); return json({ok:false},401) }
      let upd; try{ upd = await req.json() }catch{ return json({ok:false,err:'bad_json'},400) }
