...
import { getUserTokens, putUserTokens, userSaveUrl } from "./lib/userDrive.js";
...
async function setDriveMode(env, userId, on) {
  await ensureState(env).put(DRIVE_MODE_KEY(userId), on ? "1" : "0", {
    expirationTtl: 3600,
  });
}
...
function detectAttachment(msg) {
  ...
  if (msg.document) {
    ...
    return {
      type: "document",
      file_id: d.file_id,
      name: d.file_name || `document_${d.file_unique_id}`,
    };
  }
  if (msg.video) {
    const v = msg.video;
...
async function handleIncomingMedia(env, chatId, userId, msg) {
  ...
  if (!ut?.refresh_token) {
    await TG.text(
      chatId,
      "Щоб зберігати у свій Google Drive — спочатку натисни «Google Drive» і дозволь доступ.",
      { token: env.BOT_TOKEN }
    );
    return true;
  }
  ...
  await TG.text(chatId, `✅ Збережено на твоєму диску: ${saved.name}`, {
    token: env.BOT_TOKEN,
  });
  return true;
}
...
const inlineOpenDrive = () => ({
  inline_keyboard: [
    [{ text: "Відкрити Диск", url: "https://drive.google.com/drive/my-drive" }],
  ],
});
...
async function installCommandsMinimal(env) {
  ...
  await TG.setCommands(
    env.BOT_TOKEN,
    { type: "chat", chat_id: Number(env.TELEGRAM_ADMIN_ID) },
    [
    ...
    try {
      ...
      if (p === "/ci/deploy-note") {
        ...
        const commit = url.searchParams.get("commit") || "",
          ...
          depId = url.searchParams.get("deploy") || env.DEPLOY_ID || "",
          status = url.searchParams.get("status") || "";
        const line = await logDeploy(env, {
          source: "ci",
          commit,
          actor,
          deployId: depId,
      ...
      if (p === "/webhook" && req.method === "POST") {
        ...
        if (text === "/start") {
          await safe(async () => {
            ...
            await TG.text(chatId, "Привіт! Я Senti 🤖", {
              token: env.BOT_TOKEN,
        ...
        if (text === BTN_DRIVE) {
          await safe(async () => {
            ...
            await TG.text(chatId, "Переглянути вміст диска:", {
              token: env.BOT_TOKEN,
              reply_markup: inlineOpenDrive(),
            });
          });
          return json({ ok: true });
        }
        ...
        if (text === BTN_CHECK) {
          await safe(async () => {
            ...
            const link = abs(
              env,
              `/admin/checklist/html?s=${encodeURIComponent(env.WEBHOOK_SECRET || "")}`
            );
...
• /admin_snapshot — env snapshot`,
        ...
        if (text === "/admin_snapshot") {
          await safe(async () => {
            if (!ADMIN(env, userId)) return;
            await SentiCore.snapshot(env);
            await TG.text(chatId, `📦 Snapshot додано у чеклист`, {
              token: env.BOT_TOKEN,
            });
          });
          return json({ ok: true });
        ...
        if (text.startsWith("/admin_setwebhook")) {
          await safe(async () => {
            ...
            await TG.text(
              chatId,
              `✅ Вебхук → ${target}${env.TG_WEBHOOK_SECRET ? " (секрет застосовано)" : ""}`,
              { token: env.BOT_TOKEN }
            );
          });
          return json({ ok: true });
        ...
        if (text.startsWith("/admin_refreshcheck")) {
          await safe(async () => {
            if (!ADMIN(env, userId)) return;
            try {
              await appendChecklist(env, "refreshcheck ok");
              await TG.text(chatId, "✅ KV OK (append)", { token: env.BOT_TOKEN });
            } catch (e) {
              await TG.text(chatId, `❌ KV failed: ${String(e)}`, {
                token: env.BOT_TOKEN,
              });
            }
          });
        ...
        try {
          if (await getDriveMode(env, userId)) {
            if (await handleIncomingMedia(env, chatId, userId, msg))
              return json({ ok: true });
          }
        } catch (e) {
          try {
            await TG.text(
              chatId,
              `❌ Не вдалось зберегти вкладення: ${String(e)}`,
              { token: env.BOT_TOKEN }
            );
          } catch {}
          return json({ ok: true });
