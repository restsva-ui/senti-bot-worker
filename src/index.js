...
export default {
  async fetch(req, env) {
    ...
    try {
      ...
      // file -> archive -> link into checklist
      ...
      if (p === "/admin/repo/html") {
        ...
        const list = keys.map(k =>
          `<li><a href="/admin/archive/get?key=${encodeURIComponent(k)}${q}">${k}</a> — <a href="/admin/archive/delete?key=${encodeURIComponent(k)}${q}" onclick="return confirm('Delete?')">🗑</a></li>`
        ).join("") || "<li>Порожньо</li>";
...
• /admin_note_deploy — тестова деплой-нотатка`,
        ...
        if (text.startsWith("/admin_refreshcheck")) {
          await safe(async () => {
            ...
            try {
              ...
              await TG.text(chatId, `❌ KV failed: ${String(e)}`, { token: env.BOT_TOKEN });
            }
          });
          return json({ ok:true });
        }
  ...
  async scheduled(event, env, ctx) {
    ctx.waitUntil((async () => {
      try { await logHeartbeat(env); } catch {}
    })());
  }
};