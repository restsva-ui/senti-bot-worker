// src/services/ask.ts
// Smart /ask router: OpenRouter → Gemini → Cloudflare Workers AI (fallback)
...
/* -------------------- OpenRouter -------------------- */
async function askOpenRouter(env: AskEnv, prompt: string, signal: AbortSignal): Promise<AskResult> {
...
/* -------------------- Gemini -------------------- */
async function askGemini(env: AskEnv, prompt: string, signal: AbortSignal): Promise<AskResult> {
...
async function askCloudflare(env: AskEnv, prompt: string, signal: AbortSignal): Promise<AskResult> {
  const models = [
...
/* -------------------- Публічна функція -------------------- */
export async function smartAsk(env: AskEnv, prompt: string): Promise<AskResult> {
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort("ask-timeout"), 25_000);

  try {
    if (getEnvKey(env, "OPENROUTER_API_KEY", "OR_API_KEY")) {
      try { return await askOpenRouter(env, prompt, controller.signal); } catch {}
    }
    if (getEnvKey(env, "GEMINI_API_KEY", "GOOGLE_API_KEY")) {
      try { return await askGemini(env, prompt, controller.signal); } catch {}
    }
    return await askCloudflare(env, prompt, controller.signal);
  } finally {
