...
// STT router v2: OpenRouter (whisper-large-v3) → Cloudflare Whisper → Gemini.
// Фікси: коректний MIME для Telegram voice, ретраї для CF (file↔audio), гнучка підтримка Gemini v1/v1beta і -latest.
...
/* ── OpenAI-compatible (OpenRouter як перший) ─────────────────────────────── */
...
async function transcribeViaGemini(env, fileUrl) {
  ...
  try { return await geminiOnce({ key, model, fileUrl, apiBase: base }); }
    ...
    try { return await geminiOnce({ key, model, fileUrl, apiBase: altBase }); }
      ...
      const safe = "gemini-1.5-pro"; // альтернатива, якщо flash недоступний
...
export async function transcribeVoice(env, fileUrl) {
  ...
  // 1) OpenRouter — першим (найнадійніший з твоїми змінними)
