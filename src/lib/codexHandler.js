  ...
  if (cbData === CB.IDEA) {
    ...
    await sendPlain(
      ...
      [
        ...
        "Я як Senti Codex Architect поставлю уточнюючі питання, сформую короткий структурований опис (до 1–2 екранів) і попрошу підтвердження перед збереженням.",
      ].join("\n")
    );
    return true;
  }
...
export async function handleCodexGeneration(env, ctx, helpers) {
  ...
  if (awaiting === "idea_text" && textRaw) {
    ...
    const system = [
      ...
      "- загальний обсяг — орієнтовно 300–400 слів (1–2 екрани в Telegram на телефоні);",
  ...
  if (awaiting === "idea_confirm" && textRaw) {
    ...
    const system2 = [
      ...
      "- у кожному розділі не більше 2–3 маркованих пунктів;",
      "- загальний обсяг — орієнтовно 300–400 слів;",
