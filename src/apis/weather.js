...
async function geocode(place, lang = "uk") {
  const url = `${OM_GEOCODE}?name=${encodeURIComponent(
    place
...
/** Безпечне округлення та вивід значень */
function fmtNum(n) {
  const x = Number(n);
...
function summarizeWeather(json, lang = "uk") {
  ...
  const langKey = String(lang || "uk").slice(0, 2);

  // ГОЛОВНЕ: використовуємо шаблонні рядки ТІЛЬКИ всередині backticks — жодних лишніх апострофів
  return `${icon} ${d(langKey)}. Температура близько ${fmtNum(
    curT
  )}°C. Вітер ${fmtNum(wind)} м/с.`;
}

/** Допоміжне: стабільне погодне посилання */
function weatherDeepLink(lat, lon) {
  // Windy: формат "?lat,lon,zoom"
  const ll = `${Number(lat).toFixed(3)},${Number(lon).toFixed(3)},8`;
...
export async function weatherSummaryByPlace(env, userText, lang = "uk") {
  ...
  const pre = { uk: "У", ru: "В", en: "In", de: "In", fr: "À" }[
    String(lang).slice(0, 2)
  ] || "У";

  // додаємо місто до початку рядка
  const patched = out.text.replace(/^([^\s]+)/, `$1 ${pre} ${name}`);
  return { text: patched, mode: out.mode, timezone: out.timezone };
}
