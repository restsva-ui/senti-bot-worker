...
/**
 * Системна інструкція (одна мова, лаконічно, без мета-коментів).
 */
  ...
  switch (lang) {
    ...
    case "ru":
      return [
        ...
        "Коротко и дружелюбно: приветствия/«да/нет» — 1 строка; объяснения — 1–2 коротких предложения.",
        "Не переводить и не комментировать язык запроса, без преамбул. Списки — маркерами (до 5).",
        "Если данных мало — задай ровно 1 уточняющий вопрос. Без выдумок.",
      ].join(" ");
    case "de":
      return [
        "Antworte ausschließlich auf Deutsch.",
        "Kurz und freundlich: Gruß/Ja–Nein – 1 Zeile; Erklärungen – 1–2 kurze Sätze.",
        "Keine Übersetzung und keine Kommentare zur Eingangssprache. Listen mit Aufzählungen (max. 5).",
    ...
    default:
      return [
        ...
        "Do not translate or comment on the input language. Use bullet points (up to 5) for lists.",
...
export function normalizeLang(input: string, tgLanguageCode?: string): Lang {
  ...
  // 1) Дуже короткий текст — fallback на Telegram або EN
  ...
  // 4) Переможець за балами з порогом
